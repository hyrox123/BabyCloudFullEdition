...
@interface scheduleViewController()<StyledTableViewCellDelegate, UITableViewDataSource, UITableViewDelegate>
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:[NSString stringWithFormat:@"%@-KCBSubjectUnRead", [HttpService getInstance].userId]];
}
...
- (void)setupRefresh
{
    ...
    [_newsTable addHeaderWithTarget:self action:@selector(headerRereshing) dateKey:@"scheduleTable"];
    [_newsTable headerBeginRefreshing];
...
- (void)headerRereshing
{
    if (_isLoading == NO && _totalPage == 0)
    {
        ...
        [[HttpService getInstance] queryNotice:1 noticeType:@"KCB" andBlock:^(NSMutableArray *noticeArray, NSDictionary *dict) {
...
- (void)footerRereshing
{
    ...
    {
        _isLoading = YES;
        [[HttpService getInstance] queryNotice:_currentPage+1 noticeType:@"KCB" andBlock:^(NSMutableArray *noticeArray, NSDictionary *dict) {
...
- (void)dealloc
{
    ...
    NSLog(@"scheduleViewController dealloc");
}
