...
@interface recipesViewController ()<StyledTableViewCellDelegate, UITableViewDataSource, UITableViewDelegate>
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:[NSString stringWithFormat:@"%@-MZSPSubjectUnRead", [HttpService getInstance].userId]];
}
...
- (void)viewDidLoad {
    ...
    titleLable.text = @"每周食谱";
    
    self.navigationItem.titleView = titleLable;
    
    if ([[HttpService getInstance].userExtentInfo.privilege isEqualToString:@"1"] || [[HttpService getInstance].userExtentInfo.privilege isEqualToString:@"2"])
    {
...
- (void)setupRefresh
{
    ...
    [_newsTable addHeaderWithTarget:self action:@selector(headerRereshing) dateKey:@"recipesTable"];
    [_newsTable headerBeginRefreshing];
...
- (void)headerRereshing
{
    if (_isLoading == NO && _totalPage == 0)
    {
        ...
        [[HttpService getInstance] queryNotice:1 noticeType:@"MZSP" andBlock:^(NSMutableArray *noticeArray, NSDictionary *dict) {
...
- (void)footerRereshing
{
    ...
    {
        _isLoading = YES;
        [[HttpService getInstance] queryNotice:_currentPage+1 noticeType:@"MZSP" andBlock:^(NSMutableArray *noticeArray, NSDictionary *dict) {
...
- (void)dealloc
{
    ...
    NSLog(@"recipesViewController dealloc");
}
