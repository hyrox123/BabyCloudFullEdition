...
+ (void)drawImage:(OHAttributedLabel *)label
{
    for (NSArray *info in label.imageInfoArr) {
        ...
        imageView.frame = CGRectMake(imageView.frame.origin.x, imageView.frame.origin.y+3, imageView.frame.size.width, imageView.frame.size.height);
...
+ (NSMutableArray *)addAutor:(NSString *)text
{
    //匹配发布者姓名
    NSMutableArray *resultArray = [NSMutableArray new];
    NSArray *tmpArray1 = [text componentsSeparatedByString:@" "];
    
    for (NSString *subString in tmpArray1) {
        NSArray *tmpArray2 = [subString componentsSeparatedByString:@":"];
        
        if (tmpArray2.count >= 2) {
            [resultArray addObject:tmpArray2[0]];
        }
    }
